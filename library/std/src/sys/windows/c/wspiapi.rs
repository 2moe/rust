...
use crate::{
    ...
    ptr,
    sys::c::{
        in_addr, sockaddr_in, WSAGetLastError, ADDRESS_FAMILY, ADDRINFOA, AF_INET, PCSTR, PSTR,
...
unsafe fn wspiapi_strdup(string: PCSTR) -> PSTR {
    if string.is_null() {
        ...
        CStr::from_ptr(string.cast()).to_owned().into_raw().cast()
    }
}
...
compat_fn_lazy! {
    ...
    pub fn getservbyname(name: PCSTR, proto: PCSTR) -> *const servent;
    ...
    /// function calls.
    pub fn gethostbyname(name: PCSTR) -> *const hostent;
    pub fn inet_addr(cp: PCSTR) -> u32;
    pub fn inet_ntoa(r#in: in_addr) -> PSTR;
}
